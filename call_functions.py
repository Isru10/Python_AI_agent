...
def call_function(function_call_part, verbose=False):
    if verbose:
        print (f"Function call requested: {function_call_part.name} with arguments: {function_call_part.arguments}")
    else: 
        print (f"Function call requested: {function_call_part.name}")
        ...
        return types.Content(
            ...
            parts = [
                types.Part.from_function_response(
                    ...
                    response = {"error": f"Function {function_call_part.name} ."}
                )
            ]
        )
    ...
    return types.Content(
                ...
                parts = [
                    types.Part.from_function_response(
                        name = function_call_part.name,
                        response = {"result": f"Function {result} ."}
                    )
                ]
            )
